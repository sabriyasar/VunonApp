...
export const useTiming = ({
  ...
  return block([cond(isTimingInterrupted, finishTiming), cond(eq(state, State.ACTIVE), cond(and(not(and(eq(animatedStaticIndex, 0), lessThan(valueClamp, 0))), not(and(eq(animatedStaticIndex, size - 1), greaterThan(valueClamp, 0)))), [set(animationState.finished, 0), set(animationState.position, add(animatedStaticIndex, valueClamp))])), cond(eq(state, State.END), [cond(and(not(clockRunning(clock)), not(animationState.finished)), [cond(or(shouldReset, not(shouldAnimate)), set(config.toValue, animatedStaticIndex), cond(shouldAnimateNext, set(config.toValue, add(animatedStaticIndex, 1)), set(config.toValue, sub(animatedStaticIndex, 1)))), set(animationState.finished, 0), set(animationState.frameTime, 0), set(animationState.time, 0), startClock(clock)]), timing(clock, animationState, config), cond(animationState.finished, finishTiming)]), animationState.position]);
};
